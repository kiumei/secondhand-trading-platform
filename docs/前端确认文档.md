...
| A1 | 收藏 | 前端有，后端无 | 无 | 待确认 |
| A2 | 公告 | 前端有，后端无 | 无 | 待确认 |
| A3 | 站点设置 / 初始化向导 | 前端有，后端无 | 无 | 待确认 |
| A4 | 商品多图上传/展示 | 前端有，后端无 | 无 | 待确认 |
| A5 | 两级分类 | 前端有 | 无（单层） | 待确认 |
| A6 | 校园服务入口（技能服务/回收） | 前端有 | 无 | 待确认 |
| A7 | 想要数 | 前端有 | 无 | 待确认 |
| A8 | 浏览量 | 前端有 | 无 | 待确认 |
| A9 | 包邮 | 前端有 | 无 | 待确认 |
| A10 | 所在地（级联选择） | 前端有 | 无 | 待确认 |
| A11 | 在线状态 | 前端有 | 无 | 待确认 |
| A12 | 求购（type='want'） | 前端有 | 无（tradeType 为交易方式） | 待确认 |
...
| # | 功能 | 文档是否明确 | 建议 |
|---|------|------------|------|
| B1 | 仪表盘（商品/用户/订单/销售额统计） | 未明确 | 待确认 |
| B2 | 用户管理（封禁/解封/重置密码） | 未明确 | 待确认 |
| B3 | 评价管理 | 未明确 | 待确认 |
| B4 | 订单管理 | 未明确 | 待确认 |
| B5 | 公告管理 | 无 | 待确认 |
| B6 | 系统设置 | 无 | 待确认 |
...
| # | 事项 | 建议 |
|---|------|------|
| C1 | 字段名统一为后端/文档口径（goodsId/sellPrice/tradeType/goodsDesc/qualityLevel/goodsStatus 等） | 待确认 |
| C2 | 状态码由字符串改为数字（商品 0-4、订单 0-5） | 待确认 |
| C3 | 登录标识由 username 改为 phone | 待确认 |
| C4 | 认证由 localStorage 存 user 对象改为 Cookie Session + CSRF | 待确认 |

### D. 其他

| # | 事项 | 建议 |
|---|------|------|
| D1 | 主题切换（浅色/深色）——纯 UI、无数据模型 | 保留（建议） |
| D2 | 评价绑定订单（前端当前 orderId 传 0） | 待确认 |
