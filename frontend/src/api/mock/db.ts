...
function seedCategories(): Category[] {
  return [
    { categoryId: 1, cateName: '学习考试', parentId: 0 },
    { categoryId: 2, cateName: '数码装备', parentId: 0 },
    { categoryId: 3, cateName: '宿舍生活', parentId: 0 },
    { categoryId: 4, cateName: '穿搭休闲', parentId: 0 },
    { categoryId: 11, cateName: '教材文具', parentId: 1 },
    { categoryId: 12, cateName: '书籍资料', parentId: 1 },
    { categoryId: 21, cateName: '手机电脑', parentId: 2 },
    { categoryId: 22, cateName: '数码配件', parentId: 2 },
    { categoryId: 31, cateName: '宿舍用品', parentId: 3 },
    { categoryId: 32, cateName: '日常个护', parentId: 3 },
    { categoryId: 41, cateName: '运动服饰', parentId: 4 },
    { categoryId: 42, cateName: '零食美妆', parentId: 4 },
  ]
}

// items: [goodsId, title, desc, sellPrice, cateId, cateName, sellerId, qualityLevel, tradeType, goodsStatus]
function seedGoods(): Goods[] {
  const items: Array<[number, string, string, number, number, string, number, number, number, number]> = [
  ...
  return items.map(
    ([goodsId, title, goodsDesc, sellPrice, cateId, cateName, sellerId, qualityLevel, tradeType, goodsStatus], i) => ({
      ...
      images: realImages[goodsId] ?? [placeholder(goodsId + 200), placeholder(goodsId + 300), placeholder(goodsId + 400)],
      views: 20 + ((goodsId * 37) % 300),
    }),
  )
}
...
function seedOrders(): Order[] {
  return [
    { orderId: 1, goodsId: 1, buyerId: 3, sellerId: 2, orderPrice: 35, orderStatus: 3, createTime: '2026-09-02 10:00', shippingAddress: '前湖校区南区（医学部）·医学-8栋' },
    { orderId: 2, goodsId: 2, buyerId: 4, sellerId: 2, orderPrice: 2800, orderStatus: 2, createTime: '2026-09-05 14:20', shippingAddress: '青山湖校区·北区学生宿舍7栋' },
    { orderId: 3, goodsId: 5, buyerId: 3, sellerId: 2, orderPrice: 199, orderStatus: 1, createTime: '2026-09-06 09:15', shippingAddress: '前湖校区南区（医学部）·医学-8栋' },
  ]
}
...
function seedReports(): Report[] {
  return [
    { reportId: 1, reportUserId: 3, goodsId: 12, reportType: '商品与描述不符', reportContent: '收到的商品与页面描述严重不符', handleStatus: 0 },
  ]
}
